...
const char *msgs[] = { "0. Quit", "1. Add", "2. Find", "3. Delete", "4. Show", "5. Max", "6. Direct", "7. Show tree", "8. Load table from file" };
...
void getmax(Node *x)
{
	while ( x )
	{
		if ( x->right && x->right->key > x->key )
			x = x->right;
		else if ( x->left && x->left->key > x->key )
			x = x->left;
		else
		{
			printf("max: %d, info: '%s'\n", x->key, x->info);
...
int (*fptr[])(Node **) = {NULL, d_add, d_find, d_delete, d_show, d_max, d_direct_show, d_showtree, d_fload};
